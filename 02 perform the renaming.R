...
#'    to update the names of wav files and xml files in selected folders. Also if 
#'    a folder of Acoustic Pipeline results csv files is found, to update the file
#'    names listed in those files to the corrected files also.
#'    
#' Usage:
...
#'    3. When prompted, select the folder containing the audio files where present in individual site/date folders
#'    4. When prompted, select the folder containing the audio files that has been merged into site folders
#'    5. When prompted, select the folder containing the "DONE" audio files
#'    6. Wait...this step will be slow as it will be scanning thousands of files. Updates will be shown on screen. 
...
path_renamed_unmerged <- rstudioapi::selectDirectory(path = dirname(file_names), caption='Select the folder containing renamed audio in folder...')
path_renamed_merged <- rstudioapi::selectDirectory(path = dirname(file_names), caption='Select the folder containing renamed audio merged...')
path_renamed_legacy <- rstudioapi::selectDirectory(path = dirname(file_names), caption='Select the folder containing renamed audio already processed pre Mara...')
paths <- c(path_renamed_unmerged, path_renamed_merged, path_renamed_legacy)
cat('About to process the following folders:\n  ', path_renamed_unmerged,'\n  ', path_renamed_merged,'\n  ', path_renamed_legacy,'\n')
